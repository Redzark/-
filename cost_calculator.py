...
def parse_part_list_matrix(file):
    try:
        ...
        # [핵심] Qty 컬럼 자동 감지 (헤더 이름 + 데이터 존재 여부)
        # 헤더 오른쪽에 있고, 데이터에 1이나 ●가 찍힌 열을 찾음
        qty_candidates = []
        ...
        # 3. 데이터 파싱 (Active Parent Logic)
        assy_dict = {} # { "ASSY_NAME": [items...] }
        active_parents = {c: None for c in col_map['qty_cols']}
        
        debug_log = [f"✅ 헤더 위치: {header_row_index+1}행", f"ℹ️ 감지된 수량 열 개수: {len(qty_candidates)}개"]
            ...
            for q_col in col_map['qty_cols']:
                ...
                if is_root:
                    if u_val > 0: # 이 기둥에서 활성화됨
                        ...
                        active_parents[q_col] = None # 이 기둥에선 끊김
...
# 5. UI
# ============================================================================
st.set_page_config(page_title="원가계산서(Final)", layout="wide")
st.title("원가계산서 (강력 스캔 버전)")
...
if uploaded_file:
    if st.button("🔄 데이터 분석 실행"):
        data, info, logs = parse_part_list_matrix(uploaded_file)
        with st.expander("📊 분석 로그 확인"):
            for l in logs: st.write(l)
        
        if data:
            st.session_state.assy_dict = data
            st.session_state.common_car = info.get('car', '')
            st.session_state.common_vol = info.get('vol', 0)
            st.success(f"✅ {len(data)}개의 ASSY 세트를 성공적으로 분리했습니다!")
        else:
            st.error("데이터를 찾지 못했습니다. 로그를 확인해주세요.")

if st.session_state.assy_dict:
    st.markdown("### 🛠️ 생성될 파일 목록")
    for k, v in st.session_state.assy_dict.items():
        st.write(f"- 📄 **{k}** (포함 부품: {len(v)}개)")
    
    if st.button("엑셀 파일 일괄 생성 (ZIP)", type="primary"):
        z_buf = io.BytesIO()
        with zipfile.ZipFile(z_buf, "w") as zf:
            for name, items in st.session_state.assy_dict.items():
                xb = generate_excel_file_stacked(
                    {"car": st.session_state.common_car, "base_vol": int(st.session_state.common_vol)}, 
                    items, 2026
                )
                if xb: zf.writestr(f"{name}_Calculation.xlsx", xb)
        
        st.download_button("📥 전체 다운로드", z_buf.getvalue(), "Cost_Calculation_Set.zip", "application/zip")
